...
data VestingDatum = VestingDatum
    { beneficiary1 :: PubKeyHash -- person who is going to obtain the gifted ada
    , beneficiary2 :: PubKeyHash -- person who is doing the 'gifting' of the ada, will take it back after its passed.
    , deadline     :: Slot
    } deriving Show
...
mkValidator dat () ctx 
    ...
    |  (beneficiary2 dat `elem` sigs) && (from (1 + deadline dat) `contains` range) = True -- 1+ deadline would be exactly 1 tick after the deadline, hence its passed.
    |   otherwise                                                                    = False
 where
    info :: TxInfo
...
mkValidator :: VestingDatum -> () -> ScriptContext -> Bool
mkValidator dat () ctx =
    | checkDeadline == True = traceIfFalse "This is not bene2" (checkSig (beneficiary2 dat))
    | otherwise = traceIfFalse "this is not bene1" (checkSig (beneficiary1 dat))
  where
   info :: TxInfo
   info = scriptContextTxInfo ctx
   ...
   checkDeadline :: Bool
   checkDeadline = from (deadline dat) `contains` txInfoValidRangeInfo
