...
mkValidator dat () ctx
    | (beneficiary1 dat `elem` signatures) && (to         (deadline dat) `contains` range) = True
    | (beneficiary2 dat `elem` signatures) && (from (1 + deadline dat) `contains` range)   = True
    | otherwise = False
 where
    info :: TxInfo
...
mkValidator :: VestingDatum -> () -> ScriptContext -> Bool
mkValidator d () ctx = traceIfFalse "beneficiary's signature missing" signedByBeneficiary1 && traceIfFalse "deadline reached you can't grab it" insideDeadline 
                        ||
                       traceIfFalse "beneficiary's signature missing" signedByBeneficiary2 && traceIfFalse "the deadline is not reached " afterDeadline
                        = if (signedByBeneficiary1 && insideDeadline) == True
  ...
  where
    ...
    insideDeadline :: Bool
    insideDeadline = contains (to $ deadline d) $ txInfoValidRange info

    afterDeadline :: Bool
    afterDeadline = not $ contains (to $ deadline d) $ txInfoValidRange info
