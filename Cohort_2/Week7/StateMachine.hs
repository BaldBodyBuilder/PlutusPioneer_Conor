...
data GameDatum = GameDatum ByteString (Maybe GameChoice) | Finished -- finished represents final state of the state machine
    deriving Show
...
transition game s r = case (stateValue s, stateData s, r) of -- s for state datum and redeemer, the introduction of the state machine allows for a much more streamlined mkGameValidator
...
--State machine client for us to interact with it from our wallet.
...
firstGame fp = do
    ...
    void $ mapError' $ runInitialise client (GameDatum bs Nothing) v -- creates a NFT then puts in a utxo and consumes first move
