...
data Game = Game
    { gFirst          :: !PubKeyHash --player 1
    , gSecond         :: !PubKeyHash -- player 2
    , gStake          :: !Integer --player number of ADA to be used
    , gPlayDeadline   :: !POSIXTime -- by what time the second player must make a move or forfeit
    , gRevealDeadline :: !POSIXTime -- time which the first person must reveal their nonce otherwise player 2 wins
    , gToken          :: !AssetClass    -- arbitrary NFT to identify the right instance of the game
    } deriving (Show, Generic, FromJSON, ToJSON, Prelude.Eq, Prelude.Ord)

PlutusTx.makeLift ''Game

data GameChoice = Zero | One -- two moves that players can make
    deriving (Show, Generic, FromJSON, ToJSON, ToSchema, Prelude.Eq, Prelude.Ord)

instance Eq GameChoice where -- Need plutus Eq so have to manually define and need the inlineable for it to work
    {-# INLINABLE (==) #-}
    Zero == Zero = True
    One  == One  = True
    _    == _    = False
...
data GameDatum = GameDatum ByteString (Maybe GameChoice) --Starts as a nothing then a just once the second player moves
    deriving Show
...
data GameRedeemer = Play GameChoice | Reveal ByteString | ClaimFirst | ClaimSecond -- BS arg for reveal is the nonce, second player doesnt make a move, if first player doesnt reveal then the claim second allows player 2 to win.
    deriving Show
...
gameDatum o f = do
    dh      <- txOutDatum o
    Datum d <- f dh -- similar to what was done with the oracle.
...
mkGameValidator game bsZero' bsOne' dat red ctx =
    ...
    case (dat, red) of
        ...
        (GameDatum bs (Just c), Reveal nonce) ->    -- First player has won
        ...
        (GameDatum _ Nothing, ClaimFirst) ->            -- second player hasn't moved by the initial deadline, returning your money
            traceIfFalse "not signed by first player"    (txSignedBy info (gFirst game))                                    &&
            traceIfFalse "too early"                     (from (1 + gPlayDeadline game) `contains` txInfoValidRange info)   &&
            traceIfFalse "first player's stake missing"  (lovelaces (txOutValue ownInput) == gStake game)                   &&
            traceIfFalse "NFT must go to first player"   nftToFirst

        (GameDatum _ (Just _), ClaimSecond) ->          -- Both have moved but the first player has not revealed nonce, second wins
            traceIfFalse "not signed by second player"   (txSignedBy info (gSecond game))                                   &&
  ...
  where
    ...
    ownInput :: TxOut
    ownInput = case findOwnInput ctx of -- should always be an ownInput
    ...
    checkNonce :: ByteString -> ByteString -> GameChoice -> Bool -- First player has won and wants to prove it
    checkNonce bs nonce cSecond = sha2_256 (nonce `concatenate` cFirst) == bs -- nonce concat with choice, cSecond of game choice and cFirst is Bs
      where
        cFirst :: ByteString
...
data Gaming -- Datum and redeemer bundler
...
bsZero, bsOne :: ByteString -- arbitrary values, could be 0 or 1
bsZero = "0"
bsOne  = "1"
...
waitUntilTimeHasPassed t = do
    ...
    void $ awaitTime t >> waitNSlots 1 -- gets posix time then waits until that time has come
    s2 <- currentSlot
...
firstGame fp = do
    ...
    case m of
        ...
        Just (oref, o, dat) -> case dat of
            ...
            GameDatum _ (Just c') | c' == c -> do -- Second player chooses the same move we choose and thus loses
...
secondGame sp = do
    ...
    case m of
        Just (oref, o, GameDatum bs Nothing) -> do
            ...
            let v       = let x = lovelaceValueOf (spStake sp) in x <> x <> token -- input twice the stake and the nft
                c       = spChoice sp
