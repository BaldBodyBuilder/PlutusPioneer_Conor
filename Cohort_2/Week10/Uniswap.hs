...
data UniswapContracts =
      Init -- used for starting with basic tokens
    | UniswapStart
    | UniswapUser Uniswap.Uniswap -- parameterize the client
    deriving (Eq, Ord, Show, Generic)
    deriving anyclass (FromJSON, ToJSON)

instance Pretty UniswapContracts where
    pretty = viaShow -- init contract which 

instance Builtin.HasDefinitions UniswapContracts where -- between contract and the API
...
initContract = do
    ...
    let cs = Currency.currencySymbol cur -- Tokens abcd with 1m each, 4 m of each then sends 1m of each to be distributed
