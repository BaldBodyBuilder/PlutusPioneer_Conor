...
mkSwapValidator oracle addr pkh () ctx =
    txSignedBy info pkh || -- If the seller himself signs then we're good to go, no need for an oracle
  ...
  where
    ...
    oracleInput =
      let
        ins = [ o
              ...
              , txOutAddress o == addr -- Address of the oracle
              ]
      in
        case ins of
            [o] -> o
    ...
    hasTwoScriptInputs =
      let
        xs = filter (isJust . toValidatorHash . txOutAddress . txInInfoResolved) $ txInfoInputs info -- All script inputs, length should be two, oracle and the swap
      in
        length xs == 2
...
findSwaps oracle p = do
  ...
  where
    f :: TxOutTx -> Maybe PubKeyHash -- Use this for checking access to a PUBK
    f o = do
...
retrieveSwaps :: Oracle -> Contract w s Text () -- IF the seller changes their mind and wants the swap back.
retrieveSwaps oracle = do
    pkh <- pubKeyHash <$> ownPubKey
    xs  <- findSwaps oracle (== pkh) -- Checks to make sure the swaps belong to himself,.
    case xs of
...
useSwap :: forall w s. Oracle -> Contract w s Text () -- Actually making use of the oracle.
useSwap oracle = do
    funds <- ownFunds -- Checks own wallet and sees all funds I own. How much can I pay for these swaps.
    ...
    case m of
        ...
        Just (oref, o, x) -> do
            ...
            case find (f amt x) swaps of -- basically we're looking for a swap that we can afford!
                Nothing                -> logInfo @String "no suitable swap found"
                Just (oref', o', pkh') -> do --Takes the first most affordable swap.
                    let v       = txOutValue (txOutTxOut o) <> lovelaceValueOf (oFee oracle)
                        ...
                        tx      = Constraints.mustSpendScriptOutput oref  (Redeemer $ PlutusTx.toBuiltinData Use) <> -- Must use oracle as an input
                                  Constraints.mustSpendScriptOutput oref' (Redeemer $ PlutusTx.toBuiltinData ())  <> -- Use the oracle redeemer
...
type SwapSchema = -- Wrappers of the previously defined contracts that get offered over and over again
