...
--Arbitrary currency symbols to experiment with
...
test :: IO ()
test = runEmulatorTraceIO' def emCfg def myTrace -- Primed version so you can be more incontrol of the environment 
  where
    emCfg :: EmulatorConfig
    emCfg = EmulatorConfig $ Left $ Map.fromList [(Wallet i, v) | i <- [1 .. 10]]

    v :: Value
    v = Ada.lovelaceValueOf                    100_000_000 <> -- Everyone has 100mllln lovelace = 100 ADA
        Value.singleton assetSymbol assetToken 100_000_000

checkOracle :: Oracle -> Contract () Empty Text a -- perma check oracle value
checkOracle oracle = do
    m <- findOracle oracle
    case m of
        Nothing        -> return () -- Stop if oracle isn't there
        Just (_, _, x) -> Contract.logInfo $ "Oracle value: " ++ show x -- Log the value then wait for 1 slot then recurse
...
myTrace = do
    ...
    callEndpoint @"update" h1 1_500_000 -- init oracle to 1.5 ada 
    ...
    callEndpoint @"offer" h3 10_000_000 -- offer 1
    callEndpoint @"offer" h4 20_000_000 -- offer 2
    void $ Emulator.waitNSlots 3
    ...
    callEndpoint @"retrieve" h3 () -- Couldve left this out but helps to see
    callEndpoint @"retrieve" h4 ()
    void $ Emulator.waitNSlots 3
  where
    getOracle :: ContractHandle (Last Oracle) OracleSchema Text -> EmulatorTrace Oracle -- Immediately writes down the observeable state of the oracle
    getOracle h = do
        l <- observableState h
        case l of
