-- A mary era style monetary style policy
...
mkPolicy pkh deadline () ctx = traceIfFalse "beneficiary's signature is missing" signedByBeneficiary &&
...
curSymbol pkh deadline = scriptCurrencySymbol $ policy pkh deadline -- is not curried
