...
emCfg = EmulatorConfig (Left $ Map.fromList [(Wallet w, v) | w <- [1 .. 3]]) def def
  where
    v :: Value
    v = Ada.lovelaceValueOf 1_000_000_000 <> assetClassValue token 1000 -- everyone gets 1000ADA and 1000T
...
myTrace = do
    h <- activateContractWallet (Wallet 1) startEndpoint -- start endpoint running the token sale, wallet 1
    ...
    case m of
        ...
        Just ts -> do
            ...
            callEndpoint @"close" h1 () -- Only major adjustment?
            callEndpoint @"withdraw" h1 (40, 10_000_000) -- 25 tokens purchased
            void $ Emulator.waitNSlots 5
            --should be left with 