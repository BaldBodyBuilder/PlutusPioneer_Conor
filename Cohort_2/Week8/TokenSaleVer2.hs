...
transition ts s r = case (stateValue s, stateData s, r) of
    ...
    (_, Just _, Close)                      -> Just ( Constraints.mustBeSignedBy (tsSeller ts) -- > Solution, disregarding the TS and the actual state the moment the redeemer is called as a close then it can start closing
                                                    , State Nothing $
                                                      mempty
                                                    +)
    _                                       -> Nothing
...
useEndpoints ts = forever
                $ handleError logError
                $ awaitPromise
                $ setPrice' `select` addTokens' `select` buyTokens' `select` withdraw' `select` close' -- promise that waits until there is input then begins the contract
